...
public class NotesActivity extends BaseActivity implements
    ...
    private void moveToTrash() {
        ...
        cr.insert(TrashContract.URI_TABLE, values);
    }
    ...
    private void moveToArchive(View view, int position) {
        ...
        ContentResolver cr = this.getContentResolver();
        cr.insert(ArchivesContract.URI_TABLE, values);
    ...
    public void onLoadFinished(Loader<List<Note>> loader, List<Note> data) {
        ...
        for (final Note note : mNotes) {
                    ...
                    public void run() {
                        do {
                            ...
                            if (gfs.size() > 0) {
                                ...
                                note.setBitmap(BitmapFactory.decodeResource(getResources(), R.drawable.ic_loading));
            ...
            } else if (AppConstant.DROP_BOX_SELECTION == note.getStorageSelection()) {
                threads[threadCnt] = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        do {
                            Drawable drawable = getImageFromDropbox(mDropboxAPI,
                                    AppSharedPreferences.getDropBoxUploadPath(getApplicationContext()), note.getImagePath());
                            if (drawable != null) {
        ...
        if (cacheFile.exists()) {
            ...
            try {
                ...
                for (DropboxAPI.Entry ent : dirEnt.contents) {
                    if (ent.thumbExists && ent.fileName().startsWith(filename))
                        thumbs.add(ent);
                }
                if (thumbs.size() == 0)
                    mIsImageNotFound = true;
            ...
            return getResources().getDrawable(R.drawable.ic_loading);
        }
    }
